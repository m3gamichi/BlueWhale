...
try:
        ...
        try:
            ...
            if sys.platform.startswith("win"):
                ...
                file = filedialog.askopenfilename(
                    parent=root,
                    title=f"{name_tool} {version} - Choose an image file",
    ...
    def GetAllExif(image_path):
        ...
        try:
            ...
            exif_data["Availability"] = (
                "Available" if os.access(image_path, os.R_OK) else "Not available"
            )
            exif_data["Offline Status"] = (
                "Online" if os.path.exists(image_path) else "Offline"
            )
        except Exception as e:
